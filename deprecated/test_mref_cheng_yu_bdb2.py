...
def main():
    ...
    if len(args) < 3 or len(args) > 4:
        ...
        if options.MPI:
            ...
            import mpi
            from EMAN2db import db_open_dict
            from sp_utilities import set_arb_params
            ...
            while batch_start < image_end:
                ...
                # read batch
                tmp = EMData.read_images(args[0], range(batch_start, batch_end))
                for i, img_idx in enumerate( range(batch_start, batch_end) ):
                #    tmp_img.read_image( args[0], img_idx )
                    tmp[idx+i].set_attr('ID',img_idx)
                    original_images[idx+i] = tmp[idx+i].copy()
                #original_images[idx:idx+batch_end-batch_start] = tmp.copy()
                #for i, img_idx in enumerate( range(batch_start, batch_end) ):
                #    tmp_img.read_image( args[0], img_idx )
