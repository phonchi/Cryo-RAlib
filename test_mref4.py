...
CUDA_PATH = os.path.join(os.path.dirname(__file__),  "cuda2", "")
...
def mref_ali2d_gpu(
    ...
    aln_cfg = AlignConfig(
        ...
        numr[-3], 10,               # polar sampling params (no. of rings, no. of sample on ring)
...
def main():
    ...
    if len(args) < 3 or len(args) > 4:
        ...
        if options.MPI:
            ...
            print(myid, image_start, image_end, batch_img_num)
            ...
            for i in range(len(original_images)):
                print(myid, EMNumPy.em2numpy(original_images[i]))
            ...
            if Blockdata["myid_on_node"] in GPU_DEVICES:
                for i in range(len(original_images)):
                    print(EMNumPy.em2numpy(original_images[i]))
